...
Cepo returns a `DataFrame` by default, with each column correspond to a vector of differential stability (DS) statistic for that celltype. A higher DS statistic value denotes a gene that is more prioritized as a differentially stable gene in that given cell type. The columns denote cell types and the rows denote genes.
```{r}
ds_res
```
...
The returned output is a list of two `DataFrame`s, the first contain the statistic and the second contain the p-values.
...
```{r upset-plot}
library(UpSetR)
## Better off writing a function for this
