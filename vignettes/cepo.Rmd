...
We can also perform a plethora of downstream analyses, from gene set enrichment analyses to deconvolution of bulk RNA-seq, with the cell identity gene scores generated from the `Cepo` package. As an example, we will perform gene set enrichment analysis using the `fgsea` and  `escape` package. This example is not evaluated during vignette builds because downloading MSigDB resources can depend on external network access.
```{r gsea, message = FALSE, eval = FALSE}
library(escape)
library(fgsea)
