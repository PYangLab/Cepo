...
doLimma <- function(exprsMat, cellTypes) {
    ...
    for (i in seq_len(nlevels(cellTypes))) {
        ...
        meanPct <- do.call(DelayedArray::cbind, lapply(c(0, 1), function(i) {
            DelayedMatrixStats::rowSums2(exprsMat[, tmp_celltype == i, drop = FALSE] > 
    ...
    result <- list(stats = S4Vectors::DataFrame(sortList(statsResult)), pvalues = S4Vectors::DataFrame(sortList(pvalsResult)))
...
doTtest <- function(exprsMat, cellTypes) {
    ...
    for (i in seq_len(nlevels(cty))) {
        ...
        tt[[i]] <- t(DelayedArray::apply(exprsMat, 1, function(x) {
            ...
            return(c(stats = res$statistic, pvalue = res$p.value))
        }))
...
doWilcoxon <- function(exprsMat, cellTypes) {
    ...
    for (i in seq_len(nlevels(cty))) {
        ...
        tt[[i]] <- t(DelayedArray::apply(exprsMat, 1, function(x) {
            ...
            c(stats = res$statistic, pvalue = res$p.value)
        }))
        tt[[i]] <- as.data.frame(tt[[i]])
        tt[[i]]$adj.pvalue <- stats::p.adjust(tt[[i]]$pvalue, method = "BH")
    }
