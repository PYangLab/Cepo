...
#'   celltype_col = 'celltype'
#' )
plot_densities <- function(x, dx_list, n_genes = 2, assay = "logcounts", celltype_col, 
    ...
    stopifnot((is(x, "SummarizedExperiment") | is(x, "SingleCellExperiment")), is.character(assay), 
        length(assay) == 1L, is.character(celltype_col), length(celltype_col) == 
    ...
    gg.density.all <- lapply(celltype, function(cty) {
        ...
        listGGDensity <- lapply(seq_len(n), function(i) {
            dftoplot <- df[, c(i, ncol(df)), drop = FALSE]
            genename <- colnames(dftoplot)[[1]]
            colnames(dftoplot) <- c("gene", "celltype")
            
            
            ggBase <- ggplot2::ggplot(dftoplot, aes(x = .data$gene)) + ggplot2::facet_wrap(~celltype, 
                scales = "free_y", dir = "v") + ggplot2::ggtitle(genename) + ggplot2::theme_classic() + 
                ggplot2::theme(axis.title.x = element_blank(), axis.title.y = element_blank(), 
                  strip.background = element_blank(), strip.text.x = element_blank(), 
                  legend.position = "bottom")
            
            if (plotType == "histogram") {
                ggDensity <- ggBase + ggplot2::geom_histogram(aes(y = .data$..density..), 
                  colour = "black", fill = "white", bins = 15) + ggplot2::geom_density(alpha = 0.2, 
                  fill = my_col)
            } else {
                ggDensity <- ggBase + ggplot2::geom_density(alpha = 0.2, fill = my_col)
            }
    ...
    n <- unique(vapply(gg.density.all, length))
    gg.list <- lapply(gg.density.all, function(x) do.call(ggpubr::ggarrange, c(x, 
        ncol = n)))
